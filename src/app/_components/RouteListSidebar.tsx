...
interface RouteListSidebarProps {
  ...
  /** Function to retry trip planning after sign-in */
...
export function RouteListSidebar({
  ...
  // Auth guard for empty state sign-in prompt
  const emptyStateAuth = useRequireAuth("empty-state");
  ...
  return (
    <div className="w-80 overflow-y-auto border-r bg-white p-4">
      <div className="space-y-4">
        ...
        {/* Error state - show sign-in prompt for auth-related errors */}
        {error && (
          <>
            {!isAuthenticated && (error.message.includes("sign in") || error.message.includes("authentication") || error.message.includes("Strava")) ? (
              <StravaSignInPrompt
                title="Sign in with Strava to plan trips"
                description="Connect your Strava account to access segment data and plan your cycling trips."
                icon="🗺️"
                onSignIn={() => {
                  emptyStateAuth.requireAuth(() => {
                    // Retry trip planning after successful sign-in
                    onRetryPlanning?.();
                  });
                }}
              />
      ...
      {/* Sign-in modal for empty state */}
      <SignInModal
        isOpen={emptyStateAuth.isModalOpen}
        onClose={emptyStateAuth.onModalClose}
        triggerSource={emptyStateAuth.triggerSource}
        onSignInSuccess={emptyStateAuth.onSignInSuccess}
      />
    </div>
  );
} 